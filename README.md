# ProblemSolving

알고리즘 문제 풀이 기록.

## 2026

### 2026.03<br>
       - [C]코딩 기초 트레이닝 문자열 겹쳐쓰기
       - [C]코딩 기초 트레이닝 홀짝 구분하기
       - [C]코딩 기초 트레이닝 문자열 돌리기
       - [C]코딩 기초 트레이닝 문자열 붙여서 출력하기
       - [C]코딩 기초 트레이닝 덧셈식 출력하기
       - [C]코딩 기초 트레이닝 특수문자 출력하기
       - [C]코딩 기초 트레이닝 대소문자 바꿔서 출력하기
       - [C]코딩 기초 트레이닝 문자열 반복해서 출력하기
       - [C]코딩 기초 트레이닝 a와 b 출력하기
       - [C]코딩 기초 트레이닝 문자열 출력하기

## 2025

### 2025.07<br>
       - Lv.2 타겟 넘버
       - Lv.2 전화번호 목록
       - Lv.1 완주하지 못 한 선수

### 2025.06<br>
       - lv.2 다리를 지나는 트럭
       - lv.1 정수 내림차순으로 배치하기
       - lv.1 문자열 내림차순으로 배치하기
       - lv.1 3진법 뒤집기       
       - lv.1 정수 재곱근 판별
       - lv.1 예산
       - lv.1 실패율
       - lv.2 주차요금 계산
       - Lv.1 하샤드 수
       - Lv.1 K번째수
       - Lv.2 카펫
       - Lv.2 게임 맵 최단거리
       - Lv.2 타겟 넘버
       - Lv.2 기능 개발
       - Lv.1 로또 최고 순위와 최저 순위
       - Lv.2 타겟 넘버
       - Lv.3 단어 변환
       - Lv.2 게임 맵 최단거리
       - Lv.1 나머지가 1이 되는 수 찾기
       - Lv.1 숫자 문자열과 영단어
       - Lv.1 제일 작은 수 제거하기
       - Lv.1 K번째수
       - Lv.1 로또의 최고 순위와 최저 순위
       - Lv.2 기능 개발
       - Lv.1 같은 숫자는 싫어
       - Lv.2 JadenCase 문자열 만들기
       - Lv.2 짝지어 제거하기
       - Lv.2 가장 큰 수
       - Lv.2 H-Index
       - Lv.2 프로세스
       - Lv.2 괄호 회전하기
       - Lv.1 [정렬]K번째수
       - Lv.1 [완전탐색]최소직사각형
       - Lv.2 롤케이크 자르기
       - Lv.2 할인 행사
       - Lv.2 연속 부분 수열 합의 개수

### 2025.05<br>
       - Lv.2 예상 대진표
       - Lv.2 전화번호 목록
       - Lv.2 기능 개발
       - Lv.2 영어 끝말잇기
       - Lv.2 귤 고르기
       - Lv.2 구명보트
       - Lv.2 점프와 순간이동
       - Lv.2 카펫

## 2023

### 2023.11<br>
       - Lv.2 최솟값 만들기
       - Lv.2 JadenCase 문자열 만들기
       - Lv.2 최댓값과 최솟값
       - Lv.2 올바른 괄호

### 2023.06<br>
       - Lv.1 부족한 금액 계산하기
       - Lv.1 콜라츠 추측

<img width="70%" src="https://github.com/AngryDoggaebi/ProblemSolving/assets/120698922/dd632aa3-26a5-4bf7-9226-38c897935c82">

### 2023.05<br>
       - Lv.0 평행
       - Lv.0 옹알이
       - Lv.1 자릿수 더하기
       - Lv.0 겹치는 선분의 길이
       - Lv.0 안전지대
       - Lv.0 유한소수 판별하기
       - Lv.0 특이한 정렬
       - Lv.0 직사각형 넓이 구하기

### 2023.04<br>
       - Lv.0 삼각형의 완성조건 (2)
       - Lv.0 소인수분해
       - Lv.0 직각삼각형 출력하기
       - Lv.0 등수 매기기
       - Lv.0 외계어 사전
       - Lv.0 가까운 수
       - Lv.0 k의 개수
       - Lv.0 2차원으로 만들기
       - Lv.0 모스부호 (1)
       - Lv.0 합성수 찾기
       - Lv.0 문자열 정렬하기 (1)
       - Lv.0 진료 순서 정하기
       - 25_tiling
       - Lv.0 숨어있는 숫자의 덧셈 (1)
       - Lv.0 영어가 싫어요
       - 24_isSubsetOf
       - Lv.0 중복된 문자 제거
       - 23_bubbleSort
       - Lv.0 피자 나눠 먹기 (2)
       - Lv.0 주사위의 개수
       - Lv.0 다항식 더하기
       - Lv.0 대문자와 소문자
       - 22_fibonacci
       - Lv.0 A로 B 만들기
       - 21_largestProductOfThree
       - compressString
       - Lv.0 로그인 성공?
       - Lv.0 치킨 쿠폰
       - Lv.0 최빈값 구하기
       - Lv.0 인덱스 바꾸기
       - decryptCaesarCipher
       - numberSearch
       - 한 번만 등장한 문자
       - computeSquareRoot
       - isIsogram
       - Lv.0 약수 구하기
       - modulo
       - Lv.0 문자열 계산하기
       - Lv.0 편지

### 2023.03<br>
       - Lv.0 배열의 유사도
       - Lv.0 숫자 찾기
       - superIncreasing
       - Lv.0 n의 배수 고르기
       - 자릿수 더하기
       - readVertically
       - Lv.0 저주의 숫자 3
       - Lv.0 문자열안에 문자열
       - Lv.0 제곱수 판별하기
       - findBugInApples
       - Lv.0 세균 증식
       - removeExtremes
       - insertDash
       - Lv.0 연속된 수의 합
       - Lv.0 다음에 올 숫자
       - ABCheck
       - convertDoubleSpaceToSingle
       - convertListToObject
       - letterCapitalize
       - firstReverse
       - firstCharacter
       - powerOfTwo
       - computeWhenDouble
       - transformFirstAndLast
       - Lv.1 대충 만든 자판

### 2023.02<br>
       - Lv.1 삼총사
       - Lv.0 캐릭터의 좌표
       - Lv.0 암호 해독
       - Lv.0 가장 큰 수 찾기
       - Lv.0 OX퀴즈
       - Lv.0 문자열 정렬하기
       - Lv.0 문자열 밀기
       - Lv.0 종이 자르기
       - Lv.0 7의 개수
